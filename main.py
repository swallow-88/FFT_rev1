...
try:
    import sounddevice as sd          # p4a recipe 필요
    SD_OK = True
except Exception:
    SD_OK = False
    sd = None
...
class GraphWidget(Widget):
    ...
    def redraw(self,*_):
        ...
        if len(peaks) == 2:
            ...
            lbl = Label(text=f"Δ = {delta:.2f} Hz → {'고장' if bad else '정상'}",
...
class FFTApp(App):
    ...
    def toggle_accel(self, *_):
        ...
        if self.rt_on:
            ...
            threading.Thread(target=self._accel_fft_loop,
                             daemon=True).start()
        else:
    ...
    def _accel_fft_loop(self):
        while self.rt_on:
            time.sleep(0.5)
            if any(len(self.buf_acc[a])<64 for a in 'xyz'):
                continue
            ds, ymax = [], 0
            for a in 'xyz':
                ts, vals = zip(*self.buf_acc[a])
                sig = np.asarray(vals, float)
                sig -= sig.mean()
                sig *= np.hanning(len(sig))
                dt = (ts[-1]-ts[0])/(len(sig)-1) if len(sig)>1 else 1/128.0
                f = np.fft.fftfreq(len(sig), d=dt)[:len(sig)//2]
                v = np.abs(fft(sig))[:len(sig)//2]
                m = f <= 50
                f, v = f[m], v[m]
                v = np.convolve(v, np.ones(8)/8, 'same')
                ds.append(list(zip(f, v)))
                ymax = max(ymax, v.max())
            Clock.schedule_once(lambda *_:
                self.graph.update_graph(ds, [], f.max(), ymax))
    ...
    def _on_mic_block(self, in_data, frames, ti, status):
        if self.mic_on:
            self.buf_mic.extend(in_data[:,0])

    def _mic_fft_loop(self):
        while self.mic_on:
            time.sleep(0.25)
            if len(self.buf_mic) < 4096: continue
            sig = np.asarray(self.buf_mic, float)
            self.buf_mic.clear()
            sig -= sig.mean()
            sig *= np.hanning(len(sig))
            f = np.fft.fftfreq(len(sig), d=1/44100.0)[:len(sig)//2]
            v = np.abs(fft(sig))[:len(sig)//2]
            m = f <= 1500
            f, v = f[m], v[m]
        ...
        if ANDROID and ANDROID_API>=30:
            try:
                ...
                if not Env.isExternalStorageManager():
                    self.log("⚠️ 설정에서 ‘모든-파일’ 권한을 허용해야 합니다")
                    return
            except: pass
        if ANDROID and SharedStorage:
            try:
    ...
    def _csv_fft_thread(self):
        ...
        if len(res) == 1:
            ...
            Clock.schedule_once(lambda *_:
                self.graph.update_graph([d1,d2], diff, xm, ym))
        Clock.schedule_once(lambda *_:
